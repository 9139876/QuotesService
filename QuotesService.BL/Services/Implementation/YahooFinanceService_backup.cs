...
{
    internal class YahooFinanceService_backup //: IYahooFinanceService
    {
        ...
        public YahooFinanceService_backup(
        ...
        private static string GetQuotesURL(string tickerSymbol, DateTime? start, DateTime? end, TimeFrameEnum timeFrame)
        {
            ...
            return $"https://query1.finance.yahoo.com/v7/finance/download/{tickerSymbol}?period1={GetDateValue(start.Value)}&period2={GetDateValue(end.Value)}&interval={GetTimeFrameCode(timeFrame)}&events=history&includeAdjustedClose=true";
        }
